...
const HeadersFormSection = ({ headers, setHeaders }) => {
  ...
  const [headersKeysInputs, setHeadersKeysInputs] = useState(
    headersKeys.length ? headersKeys : [""],
  );
  const [headersValuesInputs, setHeadersValuesInputs] = useState(
    headersValues.length ? headersValues : [""],
  );
  ...
  const combineValues = (array) =>
    array.reduce((acc, curr, key) => {
      if (curr.trim() && headersValuesInputs[key].trim())
        acc[curr] = headersValuesInputs[key];
      return acc;
    }, {});
      ...
      {headersKeysInputs.map((_, key) => (
        <div className="headers-inputs__content" key={key}>
          ...
          <FormField
            ...
            placeholder={t("name")}
            callback={(e) =>
              setHeadersKeysInputs((prevState) =>
                prevState.map((item, index) =>
                  key === index ? e.target.value : item,
                ),
              )
            }
          ...
          <FormField
            ...
            placeholder={t("value")}
            callback={(e) =>
              setHeadersValuesInputs((prevState) =>
                prevState.map((item, index) =>
                  key === index ? e.target.value : item,
                ),
              )
            ...
            onBlur={changeHeadersInfo}
          />
          {headersKeysInputs.length > 1 && (
            <Button size="medium" icon onClick={removeHeader(key)}>
              <Icon name="trash" />
            </Button>
          )}
        </div>
      ))}
      {headersKeysInputs[headersKeysInputs.length - 1].trim() &&
        headersValuesInputs[headersValuesInputs.length - 1].trim() && (
          <Button className="add-header" onClick={addHeaderInfo}>
            Add
          </Button>
        )}
    </div>
