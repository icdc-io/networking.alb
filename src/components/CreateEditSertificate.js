...
const CreateEditCertificate = () => {
  ...
  const certificateUpdateStatus = useSelector(
    (state) => state.BalancerStore.certificateUpdateStatus,
  );
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setCertificatesData({
          ...certificatesData,
          [text]: reader.result,
          [path]: action === "drop" ? file.name : data[data.length - 1],
        });
      };
    }
  };
  ...
  const onChangeField = (e, item) =>
    setCertificatesData({ ...certificatesData, [item]: e.currentTarget.value });

  const createNewCertificate = () =>
    dispatch(
      createCertificate({ certificate: { ...requestBody, owner: userEmail } }),
    );
  const editCertificate = () =>
    dispatch(updateCertificate({ certificate: requestBody }, id));
  ...
  const disabledSaveBtn = () =>
    (commonName === certificate.name &&
      certificatesData.textCertificate === certificate.values?.cert &&
      certificatesData.textCaCertificate === certificate.values?.ca &&
      certificatesData.textPrivateKey === certificate.values?.key &&
      certificatesData.textDCaCertificate === certificate.values?.dest_ca) ||
    commonName === "";
  ...
  const displayCertificatesFileds = certificatesFields.map((el, index) => {
    const onClickCertificate = (index) => {
      refCertificate.current.children[
        index
      ...
      <div key={index} className="fields-wrapper">
        ...
        >
          <Grid.Row style={{ paddingBottom: "0px" }}>
            <Input
              type="text"
              value={certificatesData[el.path]}
              className="inputPath"
              action={true}
              onChange={(e) => onChangeField(e, el.path)}
            >
              <input disabled />
              <Button onClick={() => onClickCertificate(index)}>
                {t("browse")}
              </Button>
            </Input>
            <input
          ...
          <Grid.Row>
            <TextArea
              value={certificatesData[el.text]}
              className="textAreaForCert"
              onChange={(e) => onChangeField(e, el.text)}
            />
          </Grid.Row>
        </div>
      </div>
