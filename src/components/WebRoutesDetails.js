...
const WebRoutesDetails = () => {
  ...
  const traefikRouteDeleteStatus = useSelector(
    (state) => state.BalancerStore.traefikRouteDeleteStatus,
  );
  ...
  useEffect(() => {
    traefikRouteStatus === "fulfilled" &&
      route.certificate_id !== null &&
      dispatch(fetchCertificate(route?.certificate_id));
    ...
    subTitleHealthchek.map((obj, key) => {
      ...
      switch (obj.type) {
        ...
        case "boolean":
          input = (
            <Grid.Column as="h5" width={4}>
              {_.get(data, obj.path) ? t("trueCheck") : t("falseCheck")}
            </Grid.Column>
          );
          break;
        case "headers":
          input = value
            ? Object.keys(value).map((headerName, key) => (
                <Grid.Row
                  key={key}
                  className="web-routes-details-heathcheck-row-style"
                >
                  <Grid.Column width={4}>
